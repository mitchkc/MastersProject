...
namespace MastersProject.Data.Services
{
    public class PatientServiceList : IPatientService
    {
        ...
        private readonly List<SightTest> SightTests;
        private readonly List<Triage> Triages;
        ...
        public PatientServiceList()
        {
            ...
            SightTests = new List<SightTest>();
            Triages = new List<Triage>();
        }
        ...
        public List<Patient> GetPatients(string orderBy = "forename", string direction = "asc")
        {
            ...
            {
                ("forename", "asc") => ctx.Patients.OrderBy(r => r.Forename).ToList(),
                ("forename", "desc") => ctx.Patients.OrderByDescending(r => r.Forename).ToList(),

                ("surname", "asc") => ctx.Patients.OrderBy(r => r.Surname).ToList(),
                ("surname", "desc") => ctx.Patients.OrderByDescending(r => r.Surname).ToList(),

                ("age", "asc") => ctx.Patients.OrderBy(r => r.Age).ToList(),
                ("age", "desc") => ctx.Patients.OrderByDescending(r => r.Age).ToList(),

                _ => ctx.Patients.OrderBy(r => r.Forename).ToList()
            };
        ...
        public IList<SightTest> GetSightTests(int id)
        {
            var list = ctx.SightTests.Where(s => s.PatientId == id).ToList();
            return list;
        }
        ...
        // --------------------------------- Triage Operations --------------------------------------- //
        public IList<Triage> GetTriages(int id)
        {
            var list = ctx.Triages.Where(t => t.PatientId == id).ToList();
            return list;
        }
