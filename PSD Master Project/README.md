...
This .NET project ```MastersProject``` provides a solution containing separate Data, Test and Web(MVC) projects. When installed and used to create a new project, all references to MastersProject will be replaced with the name of your project.
...
1. An implementation of ```MastersProject.Data.Services.IUserService``` using EntityFramework to handle data storage/retrieval is provided via ```MastersProject.Data.Services.UserServiceDb```.
2. An implementation of ```MastersProject.Data.Services.IMailService``` using the .NET Smtp Mail provider to handle email sending is provided via ```MastersProject.Data.Services.SmtpMailService```.

Password hashing functionality is added via the ```MastersProject.Data.Security.Hasher``` class. This is used in the Data project ```UserServiceDb``` to hash the user password before storing in database.
...
## Install MastersProject

To install this solution as a mastersproject (mastersproject name is **termonbase**)

1. Download current version of the mastersproject

    ```$ git clone https://github.com/termon/DotNetMastersProjectBase.git```

2. Install the mastersproject so it can be used by ```dotnet new``` command. Use the path (i.e the directory location)to the cloned mastersproject directory without trailing '/'

    Linux/macOS

    ```$ dotnet new install /path/DotNetMastersProjectBase```

    Windows

    ```c:> dotnet new install c:\path\DotNetMastersProjectBase```

3. Once installed you can create a new project using this mastersproject.

    ```dotnet new termonbase -n SolutionName```

4. To uninstall a mastersproject (no longer can be used with dotnet new ).

    ```dotnet new uninstall /path/DotNetMastersProjectBase```
